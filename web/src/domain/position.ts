/**
 * Fractional indexing (F5.5) — a straight port of `server/src/domain/position.ts`,
 * which has the full rationale for the scheme. This file must reproduce that
 * one's behavior exactly, character for character: `protocol/fixtures/positions.json`
 * is the shared contract (F4) that a mismatch would fail, the same way it
 * guards the Kotlin port.
 *
