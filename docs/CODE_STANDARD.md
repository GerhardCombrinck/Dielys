...
Run the whole of CI locally before pushing:

```bash
scripts/verify.sh
```

It runs the same commands as `ci.yml`, in the same order, for all three
components. `FAST=1` skips the Android leg when only the TypeScript half changed.

Formatters, when something needs fixing:
...
- A push MUST be preceded by a green `scripts/verify.sh` covering every component the
  change touches. CI is a sense check on a clean machine, not the place a compile error
  is discovered.
- A branch SHOULD be pushed when it is a complete, testable change — not once per commit.
  Each push spends runner minutes and costs minutes of round trip, and a run that only
  confirms what was already verified locally buys nothing.
- CI stays the authority on anything local verification cannot see: a cold cache, a clean
  dependency install, and Linux path and file-permission behaviour. Local green means
  "worth pushing", not "cannot fail".
...
- The login response MUST NOT distinguish a wrong password from an account that does not
  exist — not in its status, its body, or its timing. Three distinct answers are an
  account-enumeration oracle.
- Access tokens MUST be presented as `Authorization: Bearer`, **including on the WebSocket
  upgrade request**. The token cannot travel in the `hello` message: [L3](#standard-l3)
  requires the Worker to authorize before anything reaches a `ListRoom`, and the Worker never
  sees messages sent after the socket is established.

#### The PBKDF2 work factor

`PASSWORD_ITERATIONS` is **10,000**, far below the ~600,000 usually recommended. This is a
measured platform constraint, not an oversight:

| iterations | time | |
|---|---|---|
| 10,000 | 4.5 ms | fits the Workers **free** plan's 10 ms CPU budget |
| 600,000 | 248 ms | needs Workers Paid (30 s CPU) |

Two things make it acceptable, and both are load-bearing:

- **Passwords MUST be generated by a password manager**, not chosen by a human. Iteration
  count exists to slow down guessing attacks on guessable secrets; nothing brute-forces 128
  bits of entropy at any work factor. This is the actual control.
- The hash is only reachable by someone who has already dumped the `UsersRoom` DO's storage.

The cost is stored **per user** (`users.password_iterations`), so raising the constant does
not invalidate existing accounts: an old hash keeps verifying at the cost it was made with
and is re-hashed at the current cost on that user's next successful login. Raising it is
therefore a one-line change plus a plan upgrade, at any time.
