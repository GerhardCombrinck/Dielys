...
That step only ever writes the `internal` track — the one track Play does not
hold for review, so a tag is installable within minutes. It published to open
testing (`beta`) for 0.5.17 to 0.5.19; that sat in review for days and left test
phones on the last internal build, so it came back. Promoting a build to closed
or open testing, and on to production, is a deliberate Play Console action, and the release is
