...
Each envelope names the device that made the write (`deviceId`) and the account
(`authorUserId`, see "Notifications for a list").
...
`Membership.notify` is the set of `NotifyEvent`s — `added`, `checked`, `deleted`, `updated` —
this member wants a phone notification for on this list. Per member, like `position`, and
empty by default: nobody is notified about a list until they choose to be, including on a
list they have just been invited to. `POST /auth/memberships/notify` with a
`SetListNotifyRequest` replaces the whole set (`[]` turns it off); no seq, no idempotency key,
the same last-write-wins reasoning as the list order. A list the caller is not on is `403`.
Only a shared list offers the choice, but the server does not enforce that: a list that stops
being shared keeps its set and simply has nobody else to be notified about.

The notification itself is never sent by the server. The wake push stays exactly as M1 has it
— `{type, listId, seq}` and nothing else — and the phone composes anything it shows from its
own replica after the sync that push starts (ADR 0012). What the server adds is:

- **`ChangeEnvelope.authorUserId`** — the account that made the write, set by the Worker from
  the authenticated caller, so a phone can leave its own account's edits (made on another
  device) out. Null on a change older than the field; null is never treated as "somebody
  else".
- **Wake priority.** A device whose account has a non-empty `notify` on the list, and did not
  make the write, is woken at FCM `high` priority; every other device at `normal`. Android
  demotes an app whose high-priority pushes do not end in a visible notification, so the
  priority goes where a notification is actually likely.

Additive: an older client ignores both fields; an older server's membership has no `notify`,
which a client reads as empty.
...
`fixtures/positions.json` holds the fractional-indexing vectors. `fixtures/auth/` holds
plain-HTTP request/response bodies, each with its own shape check.
`changes/task-created-no-author.json` is a change written before `authorUserId` existed.
